...
// note: modified pin code (egavrilov)
    ...
    else // (SUCCEEDED(hr))
    {
        ...
        for(ULONG nPinId = 0; nPinId < cPins; nPinId++)
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                {
                    DebugPrintf(TRACE_ERROR, TEXT("Pin communication value doesn't make sense [%d]"),ksPinCommunication);
                    hr = E_FAIL;
                }
            }
            ...
            if (SUCCEEDED(hr))
            {
                ...
                // If the pin is an IRP sink and outputs data, it's a capture pin
                          ...
                          (KSPIN_COMMUNICATION_BOTH == ksPinCommunication)) 
...
)
{
    ...
    CKsAudRenPin* pPin = FindViablePin(pwfx,0);
...
)
{
    ...
    CKsAudCapPin*   pPin = FindViablePin(pwfx,0);
