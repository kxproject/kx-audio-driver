...
// String's for logging.  These match up with the enumerations in stdafx.h
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (!fRes)
        {
            ...
            if (ERROR_IO_PENDING == dwError)
            {
                DWORD dwWait;
                // Wait for completion
                dwWait = ::WaitForSingleObject(overlapped.hEvent,3*1000); // no more than 3 seconds
...
HRESULT CKsIrpTarget::GetPropertySimple(
    ...
    OPTIONAL OUT ULONG  *ulReturned_)
{
    TRACE_ENTER();
    HRESULT hr = S_OK;
    ULONG ulReturned = 0;
