...
static kxparam_t default_params[LOUDCOMP_PARAMS_COUNT] = {0, 0,0,-60};


static kx_fxparam_descr _param_descr[LOUDCOMP_PARAMS_COUNT] = {
	{"Bass",   KX_FXPARAM_CB, -120, 120},
	{"Treble", KX_FXPARAM_CB, -120, 120},
	{"Vol",  KX_FXPARAM_CB, -120, 0 },
	{"SetVol",  KX_FXPARAM_CB, -120, -1 }};

int iLoudCompPlugin::describe_param(int ndx, kx_fxparam_descr *descr)
{
...
const kxparam_t *iLoudCompPlugin::get_plugin_presets()
{
 return presets;
}
...
int iLoudCompPlugin::get_param(int ndx, kxparam_t *value) // ndx corresponds to parameter id
{
	*value = _params[ndx]; 
	return 0;
}

int iLoudCompPlugin::set_param(int ndx, kxparam_t value)
{
	_params[ndx] = value;
	...
	case BASS_ID:
		F1 = BF;
		F2 = BF * pow(10., kabs(value*_params[VOL_ID]/_params[SETVOL_ID]) / 200.);
	...
	case TREBLE_ID:
		F1 = TF;
		F2 = TF / pow(10., kabs(value*_params[VOL_ID]/_params[SETVOL_ID]) / 200.);
		...
		break;
	case VOL_ID:
		set_dsp_register(PVOL, itodspr_scl(1000000, (int)(1000000*pow(10,(float)_params[VOL_ID]/20))));

		F1 = BF;
		F2 = BF * pow(10., kabs(_params[BASS_ID]*_params[VOL_ID]/_params[SETVOL_ID]) / 200.);
		...
		set_dsp_register(R_LA1, _dbl_to_dspword(A1));
				...
				F1 = TF;
		F2 = TF / pow(10., kabs(_params[TREBLE_ID]*_params[VOL_ID]/_params[SETVOL_ID]) / 200.);
	...
	if (cp) ((iLoudCompPluginDlg*) cp)->sync(ndx);

	return 0;
}

int iLoudCompPlugin::set_defaults() 
{
	set_all_params(default_params); 
	return 0;
}

int iLoudCompPlugin::set_all_params(kxparam_t* values)
{
	// initialize _params
	memcpy(_params, values, sizeof(kxparam_t) * LOUDCOMP_PARAMS_COUNT);
	for (int i = 0; i < LOUDCOMP_PARAMS_COUNT; i++) set_param(i, values[i]);
	return 0;
}

int iLoudCompPlugin::request_microcode() 
{
	publish_microcode(loudcomp);

	return 0;
}

const char *iLoudCompPlugin::get_plugin_description(int id)
{
 plugin_description(loudcomp);
}


iKXPluginGUI *iLoudCompPlugin::create_cp(kDialog *parent, kFile *mf)
{
	iLoudCompPluginDlg *tmp;
	tmp = new iLoudCompPluginDlg(this, parent,mf);
	tmp->create();
	return tmp;
}

int iLoudCompPlugin::get_user_interface()
{
...
void iLoudCompPluginDlg::init()
{
	CKXPluginGUI::init();

	// custom dialog initialization here...
	// MAKEUP_DLG(Caption, Width, Height)
	create_dialog(plugin_name(loudcomp), 240, 220,-1,KXFX_NO_RESIZE);
	...
	for (int i = 0; i < LOUDCOMP_PARAMS_COUNT; i++)
	 sync(i);

        redraw();
}

void iLoudCompPluginDlg::sync(int ndx)
{
