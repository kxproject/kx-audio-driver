...
#define LOUDCOMP_PARAMS_COUNT	4

// Plugin definition
class iLoudCompPlugin : public iKXPlugin
{
public:
// members of iKXPlugin
	...
	virtual int get_param_count(void) {return LOUDCOMP_PARAMS_COUNT;}
...
// not members of iKXPlugin
	kxparam_t _params[LOUDCOMP_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iLoudCompPluginDlg : public CKXPluginGUI
{
public:
	iLoudCompPlugin *plugin;

    	// constructor:
	iLoudCompPluginDlg(iLoudCompPlugin *plg, kDialog *parent_, kFile *mf_) : CKXPluginGUI(parent_,mf_) {plugin = plg; };
...
declare_effect_source(loudcomp);
