...
// dsp data
#include "cross4_dsp_.h" 

DECLARE_DSP_PROGRAM(dsp_pgm_data, cross4); 
...
int Cross4::get_param_count()
{
	return params_count;
}

int Cross4::describe_param(int id, kx_fxparam_descr *descr)
{
	memcpy(descr, &param_descr[id], sizeof kx_fxparam_descr); 
	return 0;
}

int Cross4::get_param(int id, kxparam_t *value)
{
	*value = params[id]; 
	return 0;
}

int Cross4::set_param(int id, kxparam_t value)
{
	params[id] = int(value);
	...
	switch(id)
	{
		case ID_FREQ:
			...
			write_gpr(R_B , double2fixed(b));
			write_gpr(R_A2, double2fixed(a2));
			write_gpr(R_A3, double2fixed(a3));
			break;
...
uPluginView* Cross4::create_view()
{
	uPluginView* pv = (uPluginView*) new Cross4View(this);
	return pv;
}
...
Cross4View::Cross4View(uPlugin* plugin)
{
	/* set plugin this view is created for */
	Cross4View::plugin = plugin;
}
...
int Cross4View::open()
{
	...
	// bkgbitmap.LoadBitmap(IDB_CROSS4BACK);
	size.right	= 272;
	size.bottom = 52;
...
int Cross4View::ucontrol_changed(int id, int data)
{
	kxparam_t value;
...
int Cross4View::param_changed(int id, kxparam_t value)
{
	switch(id)
	{
	case ID_FREQ:
