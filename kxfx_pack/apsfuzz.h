...
declare_effect_source(apsfuzz);

// Number of plugin parameters
#define APSFUZZ_PARAMS_COUNT	5

// Plugin definition
class iAPSFuzzPlugin : public iKXPlugin
{
public:
	// members of iKXPlugin
	virtual int request_microcode();
	virtual const char *get_plugin_description(int id) { plugin_description(apsfuzz); };
	
	virtual iKXPluginGUI *create_cp(kDialog *parent, kFile *);
	virtual int get_user_interface();

	virtual int get_param_count(void) {return APSFUZZ_PARAMS_COUNT;};
	...
	kxparam_t _params[APSFUZZ_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iAPSFuzzPluginDlg : public CKXPluginGUI
{
public:
	iAPSFuzzPlugin *plugin;

	// constructor:
	iAPSFuzzPluginDlg(iAPSFuzzPlugin *plg, kDialog *parent_, kFile *mf_):CKXPluginGUI(parent_, mf_) {plugin = plg;};
	...
	// Controls
	kFader fdrEQCenter;
	kFader fdrEQBandwidth;
	kFader fdrGain;
	kFader fdrPreLPCutoff;
//	kFader fdrPreLPCutoff1;
	kFader fdrEdge;
