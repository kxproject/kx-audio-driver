...
static kxparam_t Phat_EQ_Stereo_default_params[Phat_EQ_Stereo_PARAMS_COUNT] = { 10000, 120, 707, 0};

/* Plugin Implementation */

int iPhat_EQ_StereoPlugin::get_param(int ndx, kxparam_t *value) // ndx corresponds to parameter id
{
	*value = _params[ndx];
	return 0;
}

int iPhat_EQ_StereoPlugin::set_param(int ndx, kxparam_t value)
{
...
dumm = 2.;
	...
	if (cp) ((iPhat_EQ_StereoPluginDlg*) cp)->sync(ndx);

	return 0;
}

int iPhat_EQ_StereoPlugin::set_defaults()
{
	set_all_params(Phat_EQ_Stereo_default_params);
	return 0;
}

int iPhat_EQ_StereoPlugin::set_all_params(kxparam_t* values)
{
	for (int i = 0; i < Phat_EQ_Stereo_PARAMS_COUNT; i++) set_param(i, values[i]);
	return 0;
}

int iPhat_EQ_StereoPlugin::request_microcode()
{
	publish_microcode(Phat_EQ_Stereo);
	return 0;
}

iKXPluginGUI *iPhat_EQ_StereoPlugin::create_cp(kDialog *parent, kFile *mf)
{
	iPhat_EQ_StereoPluginDlg *tmp;
	tmp = new iPhat_EQ_StereoPluginDlg(this, parent, mf);
	tmp->create();
	return tmp;
}

int iPhat_EQ_StereoPlugin::get_user_interface()
{
	return IKXPLUGIN_OWNER;	// says to mixer that this plugin has its own control panel
}

// MIDI control implementation
static kx_fxparam_descr _param_descr[Phat_EQ_Stereo_PARAMS_COUNT] = {
...
int iPhat_EQ_StereoPlugin::describe_param(int ndx, kx_fxparam_descr *descr)
{
...
void iPhat_EQ_StereoPluginDlg::init()
{
	CKXPluginGUI::init();
	// custom dialog initialization here...

	// create_dialog(Caption, Width, Height, ExtraHeight = 25)
	create_dialog(plugin_name(Phat_EQ_Stereo), 420, 164);
...
void iPhat_EQ_StereoPluginDlg::sync(int ndx)
{
