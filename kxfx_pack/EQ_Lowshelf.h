...
declare_effect_source(EQ_Lowshelf);

// Number of plugin parameters
#define EQ_Lowshelf_PARAMS_COUNT	3

// Plugin definition
class iEQ_LowshelfPlugin : public iKXPlugin
{
public:
	// members of iKXPlugin
	virtual int request_microcode();
	virtual const char *get_plugin_description(int id) { plugin_description(EQ_Lowshelf); };
	
	virtual iKXPluginGUI *create_cp(kDialog *parent, kFile *);
	virtual int get_user_interface();

	virtual int get_param_count(void) {return EQ_Lowshelf_PARAMS_COUNT;}
	...
	kxparam_t _params[EQ_Lowshelf_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iEQ_LowshelfPluginDlg : public CKXPluginGUI
{
public:
	iEQ_LowshelfPlugin *plugin;

	// constructor:
	iEQ_LowshelfPluginDlg(iEQ_LowshelfPlugin *plg, kDialog *parent_, kFile *mf_):CKXPluginGUI(parent_, mf_) {plugin = plg;};
