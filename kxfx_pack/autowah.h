...
declare_effect_source(autowah);

// Number of plugin parameters
#define AUTOWAH_PARAMS_COUNT	5

// Plugin definition
class iAutoWahPlugin : public iKXPlugin
{
public:
	// members of iKXPlugin
	virtual int request_microcode();
	virtual const char *get_plugin_description(int id) { plugin_description(autowah); };
	
	virtual iKXPluginGUI *create_cp(kDialog *parent, kFile *);
	virtual int get_user_interface();

	virtual int get_param_count(void) {return AUTOWAH_PARAMS_COUNT;};
	...
	kxparam_t _params[AUTOWAH_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iAutoWahPluginDlg : public CKXPluginGUI
{
public:
	iAutoWahPlugin *plugin;

	// constructor:
	iAutoWahPluginDlg(iAutoWahPlugin *plg, kDialog *parent_, kFile *mf_):CKXPluginGUI(parent_, mf_) {plugin = plg;};
