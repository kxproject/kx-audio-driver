...
declare_effect_source(Phat_EQ_Mono);

// Number of plugin parameters
#define Phat_EQ_Mono_PARAMS_COUNT	4

// Plugin definition
class iPhat_EQ_MonoPlugin : public iKXPlugin
{
public:
	// members of iKXPlugin
	virtual int request_microcode();
	virtual const char *get_plugin_description(int id) { plugin_description(Phat_EQ_Mono); };

	virtual iKXPluginGUI *create_cp(kDialog *parent, kFile *);
	virtual int get_user_interface();

	virtual int get_param_count(void) {return Phat_EQ_Mono_PARAMS_COUNT;}
	...
	kxparam_t _params[Phat_EQ_Mono_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iPhat_EQ_MonoPluginDlg : public CKXPluginGUI
{
public:
	iPhat_EQ_MonoPlugin *plugin;

	// constructor:
	iPhat_EQ_MonoPluginDlg(iPhat_EQ_MonoPlugin *plg, kDialog *parent_, kFile *mf_):CKXPluginGUI(parent_, mf_) {plugin = plg;};
