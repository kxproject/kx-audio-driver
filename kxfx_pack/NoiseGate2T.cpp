...
// -------------------------
static kxparam_t default_params[NoiseGate2T_PARAMS_COUNT] = {0, 0, 0, 0};

int iNoiseGate2TPlugin::set_defaults() 
{
...
static kx_fxparam_descr _param_descr[NoiseGate2T_PARAMS_COUNT] = {
...
int iNoiseGate2TPlugin::describe_param(int ndx, kx_fxparam_descr *descr)
{
...
const kxparam_t *iNoiseGate2TPlugin::get_plugin_presets()
{
 return presets;
}
...
int iNoiseGate2TPlugin::get_param(int ndx, kxparam_t *value) // ndx corresponds to 'logical' parameter id
{
	*value = _params[ndx]; 
	return 0;
}
...
int iNoiseGate2TPlugin::set_param(int ndx, kxparam_t value)
{
	double x;
...
// we need do synchronize the GUI part, if set_param was called by the mixer or kX Automation
	if (cp) ((iNoiseGate2TPluginDlg*) cp)->sync(ndx);

	return 0;
}

int iNoiseGate2TPlugin::set_all_params(kxparam_t* values)
{
	for (int i = 0; i < NoiseGate2T_PARAMS_COUNT; i++) set_param(i, values[i]);
	return 0;
}

int iNoiseGate2TPlugin::request_microcode() 
{
	publish_microcode(NoiseGate2T);

	return 0;
}

const char *iNoiseGate2TPlugin::get_plugin_description(int id)
{
	plugin_description(NoiseGate2T);
}


iKXPluginGUI *iNoiseGate2TPlugin::create_cp(kDialog *parent, kFile *mf)
{
	iNoiseGate2TPluginDlg *tmp;
	tmp = new iNoiseGate2TPluginDlg(this, parent,mf);
	tmp->create();
	return tmp;
}

int iNoiseGate2TPlugin::get_user_interface()
{
...
void iNoiseGate2TPluginDlg::sync(int ndx)
{
