...
declare_effect_source(EQ_Highshelf);

// Number of plugin parameters
#define EQ_Highshelf_PARAMS_COUNT	3

// Plugin definition
class iEQ_HighshelfPlugin : public iKXPlugin
{
public:
	// members of iKXPlugin
	virtual int request_microcode();
	virtual const char *get_plugin_description(int id) { plugin_description(EQ_Highshelf); };
	
	virtual iKXPluginGUI *create_cp(kDialog *parent, kFile *);
	virtual int get_user_interface();

	virtual int get_param_count(void) {return EQ_Highshelf_PARAMS_COUNT;}
	...
	kxparam_t _params[EQ_Highshelf_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iEQ_HighshelfPluginDlg : public CKXPluginGUI
{
public:
	iEQ_HighshelfPlugin *plugin;

	// constructor:
	iEQ_HighshelfPluginDlg(iEQ_HighshelfPlugin *plg, kDialog *parent_, kFile *mf_):CKXPluginGUI(parent_, mf_) {plugin = plg;};
