...
declare_effect_source(EQ_Bandpass);

// Number of plugin parameters
#define EQ_Bandpass_PARAMS_COUNT	2

// Plugin definition
class iEQ_BandpassPlugin : public iKXPlugin
{
public:
	// members of iKXPlugin
	virtual int request_microcode();
	virtual const char *get_plugin_description(int id) { plugin_description(EQ_Bandpass); };
	
	virtual iKXPluginGUI *create_cp(kDialog *parent, kFile *);
	virtual int get_user_interface();

	virtual int get_param_count(void) {return EQ_Bandpass_PARAMS_COUNT;}
	...
	kxparam_t _params[EQ_Bandpass_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iEQ_BandpassPluginDlg : public CKXPluginGUI
{
public:
	iEQ_BandpassPlugin *plugin;

	// constructor:
	iEQ_BandpassPluginDlg(iEQ_BandpassPlugin *plg, kDialog *parent_, kFile *mf_):CKXPluginGUI(parent_, mf_) {plugin = plg;};
