...
static kxparam_t apsexp_default_params[APSEXP_PARAMS_COUNT] = {10 , 200, 0, -20, 3, 0}; 
								//{attack, release, postgain, threshold, ratio, predelay}

static kx_fxparam_descr _param_descr[APSEXP_PARAMS_COUNT] = {
...
int iAPSExpPlugin::describe_param(int ndx, kx_fxparam_descr *descr)
{
...
int iAPSExpPlugin::get_param(int ndx, kxparam_t *value) // ndx corresponds to parameter id
{
	*value = _params[ndx]; 
	return 0;
}

int iAPSExpPlugin::set_param(int ndx, kxparam_t value)
{
	...
	if (cp) ((iAPSExpPluginDlg*) cp)->sync(ndx);

	return 0;
}

int iAPSExpPlugin::set_defaults() 
{
	set_all_params(apsexp_default_params); 
	return 0;
}

int iAPSExpPlugin::set_all_params(kxparam_t* values)
{
	for (int i = 0; i < APSEXP_PARAMS_COUNT; i++) set_param(i, values[i]);
...
int iAPSExpPlugin::request_microcode() 
{
	publish_microcode(apsexp);
	return 0;
}

iKXPluginGUI *iAPSExpPlugin::create_cp(kDialog *parent, kFile *mf)
{
	iAPSExpPluginDlg *tmp;
	tmp = new iAPSExpPluginDlg(this, parent, mf);
	tmp->create();
	return tmp;
}

int iAPSExpPlugin::get_user_interface()
{
...
void iAPSExpPluginDlg::sync(int ndx)
{
