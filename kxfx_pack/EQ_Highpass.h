...
declare_effect_source(EQ_Highpass);

// Number of plugin parameters
#define EQ_Highpass_PARAMS_COUNT	2

// Plugin definition
class iEQ_HighpassPlugin : public iKXPlugin
{
public:
	// members of iKXPlugin
	virtual int request_microcode();
	virtual const char *get_plugin_description(int id) { plugin_description(EQ_Highpass); };
	
	virtual iKXPluginGUI *create_cp(kDialog *parent, kFile *);
	virtual int get_user_interface();

	virtual int get_param_count(void) {return EQ_Highpass_PARAMS_COUNT;}
	...
	kxparam_t _params[EQ_Highpass_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iEQ_HighpassPluginDlg : public CKXPluginGUI
{
public:
	iEQ_HighpassPlugin *plugin;

	// constructor:
	iEQ_HighpassPluginDlg(iEQ_HighpassPlugin *plg, kDialog *parent_, kFile *mf_):CKXPluginGUI(parent_, mf_) {plugin = plg;};
