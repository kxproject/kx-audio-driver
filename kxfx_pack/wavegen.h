...
declare_effect_source(wavegen);

// Number of plugin parameters
#define WAVEGEN_PARAMS_COUNT	2

// Plugin definition
class iWavegenPlugin : public iKXPlugin
{
public:
	// members of iKXPlugin
	virtual int request_microcode();
	virtual const char *get_plugin_description(int id) { plugin_description(wavegen); };
	
	virtual iKXPluginGUI *create_cp(kDialog *parent, kFile *);
	virtual int get_user_interface();

	virtual int get_param_count(void) {return WAVEGEN_PARAMS_COUNT;}
	...
	kxparam_t _params[WAVEGEN_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iWavegenPluginDlg : public CKXPluginGUI
{
public:
	iWavegenPlugin *plugin;
	virtual iKXPlugin *get_plugin() { return plugin; };

	// constructor:
	iWavegenPluginDlg(iWavegenPlugin *plg, kDialog *parent_, kFile *mf_):CKXPluginGUI(parent_, mf_) {plugin = plg;};
	virtual void init();
