...
static kxparam_t EQ_Notch_default_params[EQ_Notch_PARAMS_COUNT] = { 10000, 120};

/* Plugin Implementation */

int iEQ_NotchPlugin::get_param(int ndx, kxparam_t *value) // ndx corresponds to parameter id
{
	*value = _params[ndx];
	return 0;
}

int iEQ_NotchPlugin::set_param(int ndx, kxparam_t value)
{
...
switch (ndx)
	...
	if (cp) ((iEQ_NotchPluginDlg*) cp)->sync(ndx);

	return 0;
}

int iEQ_NotchPlugin::set_defaults()
{
	set_all_params(EQ_Notch_default_params);
	return 0;
}

int iEQ_NotchPlugin::set_all_params(kxparam_t* values)
{
	for (int i = 0; i < EQ_Notch_PARAMS_COUNT; i++) set_param(i, values[i]);
	return 0;
}

int iEQ_NotchPlugin::request_microcode()
{
	publish_microcode(EQ_Notch);
	return 0;
}

iKXPluginGUI *iEQ_NotchPlugin::create_cp(kDialog *parent, kFile *mf)
{
	iEQ_NotchPluginDlg *tmp;
	tmp = new iEQ_NotchPluginDlg(this, parent, mf);
	tmp->create();
	return tmp;
}

int iEQ_NotchPlugin::get_user_interface()
{
	return IKXPLUGIN_OWNER;	// says to mixer that this plugin has its own control panel
}

// MIDI control implementation
static kx_fxparam_descr _param_descr[EQ_Notch_PARAMS_COUNT] = {
	{"Frequency", KX_FXPARAM_HZ, 20, 19980},
	{"Width (Q)", KX_FXPARAM_PERCENT, 5, 2000}};

int iEQ_NotchPlugin::describe_param(int ndx, kx_fxparam_descr *descr)
{
...
void iEQ_NotchPluginDlg::sync(int ndx)
{
