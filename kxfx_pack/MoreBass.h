...
#ifndef _KX_MOREBASS_EFFECT_H
#define _KX_MOREBASS_EFFECT_H

#define MOREBASS_PARAMS_COUNT	5
										

// Plugin definition
class iMoreBassPlugin : public iKXPlugin
{
public:
	...
	virtual int get_param_count(void) {return MOREBASS_PARAMS_COUNT;}
	...
	kxparam_t _params[MOREBASS_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iMoreBassPluginDlg : public CKXPluginGUI
{
public:
	iMoreBassPlugin *plugin;

    	// constructor:
	iMoreBassPluginDlg(iMoreBassPlugin *plg, kDialog *parent_, kFile *mf_) : CKXPluginGUI(parent_,mf_) {plugin = plg; };
...
declare_effect_source(MoreBass);
