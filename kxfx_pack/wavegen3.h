...
declare_effect_source(wavegen3);

// Number of plugin parameters
#define WAVEGEN3_PARAMS_COUNT	2

// Plugin definition
class iWavegen3Plugin : public iKXPlugin
{
public:
	// members of iKXPlugin
	virtual int request_microcode();
	virtual const char *get_plugin_description(int id) { plugin_description(wavegen3); };
	
	virtual iKXPluginGUI *create_cp(kDialog *parent, kFile *);
	virtual int get_user_interface();

	virtual int get_param_count(void) {return WAVEGEN3_PARAMS_COUNT;}
	...
	kxparam_t _params[WAVEGEN3_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iWavegen3PluginDlg : public CKXPluginGUI
{
public:
	iWavegen3Plugin *plugin;
	virtual iKXPlugin *get_plugin() { return plugin; };

	// constructor:
	iWavegen3PluginDlg(iWavegen3Plugin *plg, kDialog *parent_, kFile *mf_):CKXPluginGUI(parent_, mf_) {plugin = plg;};
	virtual void init();
