...
#ifndef _KX_NoiseGate2T_EFFECT_H
#define _KX_NoiseGate2T_EFFECT_H

#define NoiseGate2T_PARAMS_COUNT	5	// number of user-visible parameters

// Plugin definition
class iNoiseGate2TPlugin : public iKXPlugin
{
public:
	...
	virtual int get_param_count(void) {return NoiseGate2T_PARAMS_COUNT;}
	...
	kxparam_t _params[NoiseGate2T_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iNoiseGate2TPluginDlg : public CKXPluginGUI
{
public:
	iNoiseGate2TPlugin *plugin;

    	// constructor:
	iNoiseGate2TPluginDlg(iNoiseGate2TPlugin *plg, kDialog *parent_, kFile *mf_) : CKXPluginGUI(parent_,mf_) {plugin = plg; };
...
declare_effect_source(NoiseGate2T);
