...
static kxparam_t EQ_Lowpass_default_params[EQ_Lowpass_PARAMS_COUNT] = { 10000, 120};

/* Plugin Implementation */

int iEQ_LowpassPlugin::get_param(int ndx, kxparam_t *value) // ndx corresponds to parameter id
{
	*value = _params[ndx];
	return 0;
}

int iEQ_LowpassPlugin::set_param(int ndx, kxparam_t value)
{
...
alpha = sn/(2*eqband);
	...
	if (cp) ((iEQ_LowpassPluginDlg*) cp)->sync(ndx);

	return 0;
}

int iEQ_LowpassPlugin::set_defaults()
{
	set_all_params(EQ_Lowpass_default_params);
	return 0;
}

int iEQ_LowpassPlugin::set_all_params(kxparam_t* values)
{
	for (int i = 0; i < EQ_Lowpass_PARAMS_COUNT; i++) set_param(i, values[i]);
	return 0;
}

int iEQ_LowpassPlugin::request_microcode()
{
	publish_microcode(EQ_Lowpass);
	return 0;
}

iKXPluginGUI *iEQ_LowpassPlugin::create_cp(kDialog *parent, kFile *mf)
{
	iEQ_LowpassPluginDlg *tmp;
	tmp = new iEQ_LowpassPluginDlg(this, parent, mf);
	tmp->create();
	return tmp;
}

int iEQ_LowpassPlugin::get_user_interface()
{
	return IKXPLUGIN_OWNER;	// says to mixer that this plugin has its own control panel
}

// MIDI control implementation
static kx_fxparam_descr _param_descr[EQ_Lowpass_PARAMS_COUNT] = {
	{"Frequency", KX_FXPARAM_HZ, 20, 20000},
	{"Width (Q)", KX_FXPARAM_PERCENT, 1, 2000}};

int iEQ_LowpassPlugin::describe_param(int ndx, kx_fxparam_descr *descr)
{
...
void iEQ_LowpassPluginDlg::init()
{
	CKXPluginGUI::init();
	// custom dialog initialization here...

	// create_dialog(Caption, Width, Height, ExtraHeight = 25)
	create_dialog(plugin_name(EQ_Lowpass), 380, 100);

	// create_hfader(Fader, Ctrl_ID, Label, MinVal, MaxVal, Left, Top, Width, Label_Width)
	create_hfader(freq, FREQ_ID, "Freq.(Hz):",20,19980, 7, 12, 363, 75);
	create_hfader(band, BAND_ID, "Width (Q):",1, 2000, 7, 48, 363, 75);
...
void iEQ_LowpassPluginDlg::sync(int ndx)
{
