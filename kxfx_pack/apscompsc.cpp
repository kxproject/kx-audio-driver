...
static kxparam_t apscomp_sc_default_params[APSCOMPSC_PARAMS_COUNT] = {10 , 200, 0, -20, 3, 0, 0};
								//{attack, release, postgain, threshold, ratio, predelay, sidechain}

static kx_fxparam_descr _param_descr[APSCOMPSC_PARAMS_COUNT] = {
...
int iAPSCompSCPlugin::describe_param(int ndx, kx_fxparam_descr *descr)
{
...
int iAPSCompSCPlugin::get_param(int ndx, kxparam_t *value) // ndx corresponds to parameter id
{
	*value = _params[ndx];
	return 0;
}

int iAPSCompSCPlugin::set_param(int ndx, kxparam_t value)
{
	...
	case SIDECHAIN_ID:
		...
		if (!value)
		{
			write_instruction(0, 0x0u , 0x800du,0x4000u,0x2040u,0x2040u);
			write_instruction(1, 0x0u , 0x800eu,0x4001u,0x2040u,0x2040u);
		}
		// if on write "read side chain" instructions
		else
		{
			write_instruction(0, 0x0u , 0x800du,0x4002u,0x2040u,0x2040u);
			write_instruction(1, 0x0u , 0x800eu,0x4003u,0x2040u,0x2040u);
		}
	...
	if (cp) ((iAPSCompSCPluginDlg*) cp)->sync(ndx);

	return 0;
}

int iAPSCompSCPlugin::set_defaults()
{
	set_all_params(apscomp_sc_default_params);
	return 0;
}

int iAPSCompSCPlugin::set_all_params(kxparam_t* values)
{
	for (int i = 0; i < APSCOMPSC_PARAMS_COUNT; i++) set_param(i, values[i]);
...
int iAPSCompSCPlugin::request_microcode()
{
	publish_microcode(apscomp_sc);
	return 0;
}

iKXPluginGUI *iAPSCompSCPlugin::create_cp(kDialog *parent, kFile *mf)
{
	iAPSCompSCPluginDlg *tmp;
	tmp = new iAPSCompSCPluginDlg(this, parent, mf);
	tmp->create();
	return tmp;
}

int iAPSCompSCPlugin::get_user_interface()
{
...
void iAPSCompSCPluginDlg::init()
{
	CKXPluginGUI::init();
	// custom dialog initialization here...

	// create_dialog(Caption, Width, Height, ExtraHeight)
	create_dialog(plugin_name(apscomp_sc), 300, 260); //w=275
...
void iAPSCompSCPluginDlg::sync(int ndx)
{
