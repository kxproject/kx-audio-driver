...
#ifndef _KX_NoiseGate2Ts_EFFECT_H
#define _KX_NoiseGate2Ts_EFFECT_H

#define NoiseGate2Ts_PARAMS_COUNT	5	// number of user-visible parameters

// Plugin definition
class iNoiseGate2TsPlugin : public iKXPlugin
{
public:
	...
	virtual int get_param_count(void) {return NoiseGate2Ts_PARAMS_COUNT;}
	...
	kxparam_t _params[NoiseGate2Ts_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iNoiseGate2TsPluginDlg : public CKXPluginGUI
{
public:
	iNoiseGate2TsPlugin *plugin;

    	// constructor:
	iNoiseGate2TsPluginDlg(iNoiseGate2TsPlugin *plg, kDialog *parent_, kFile *mf_) : CKXPluginGUI(parent_,mf_) {plugin = plg; };
...
declare_effect_source(NoiseGate2Ts);
