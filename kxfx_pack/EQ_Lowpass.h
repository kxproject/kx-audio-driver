...
declare_effect_source(EQ_Lowpass);

// Number of plugin parameters
#define EQ_Lowpass_PARAMS_COUNT	2

// Plugin definition
class iEQ_LowpassPlugin : public iKXPlugin
{
public:
	// members of iKXPlugin
	virtual int request_microcode();
	virtual const char *get_plugin_description(int id) { plugin_description(EQ_Lowpass); };
	
	virtual iKXPluginGUI *create_cp(kDialog *parent, kFile *);
	virtual int get_user_interface();

	virtual int get_param_count(void) {return EQ_Lowpass_PARAMS_COUNT;}
	...
	kxparam_t _params[EQ_Lowpass_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iEQ_LowpassPluginDlg : public CKXPluginGUI
{
public:
	iEQ_LowpassPlugin *plugin;

	// constructor:
	iEQ_LowpassPluginDlg(iEQ_LowpassPlugin *plg, kDialog *parent_, kFile *mf_):CKXPluginGUI(parent_, mf_) {plugin = plg;};
