...
declare_effect_source(amp);

// Number of plugin parameters
#define AMP_PARAMS_COUNT	3

// Plugin definition
class iAmpPlugin : public iKXPlugin
{
public:
	// members of iKXPlugin
	virtual int request_microcode();
	virtual const char *get_plugin_description(int id) { plugin_description(amp); };
	
	virtual iKXPluginGUI *create_cp(kDialog *parent, kFile *);
	virtual int get_user_interface();

	virtual int get_param_count(void) {return AMP_PARAMS_COUNT;}
	...
	kxparam_t _params[AMP_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iAmpPluginDlg : public CKXPluginGUI
{
public:
	iAmpPlugin *plugin;

	// constructor:
	iAmpPluginDlg(iAmpPlugin *plg, kDialog *parent_, kFile *mf_):CKXPluginGUI(parent_, mf_) {plugin = plg;};
