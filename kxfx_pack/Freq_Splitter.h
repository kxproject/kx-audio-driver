...
declare_effect_source(Freq_Splitter);

// Number of plugin parameters
#define Freq_Splitter_PARAMS_COUNT	2

// Plugin definition
class iFreq_SplitterPlugin : public iKXPlugin
{
public:
	// members of iKXPlugin
	virtual int request_microcode();
	virtual const char *get_plugin_description(int id) { plugin_description(Freq_Splitter); };
	
	virtual iKXPluginGUI *create_cp(kDialog *parent, kFile *);
	virtual int get_user_interface();

	virtual int get_param_count(void) {return Freq_Splitter_PARAMS_COUNT;}
	...
	kxparam_t _params[Freq_Splitter_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iFreq_SplitterPluginDlg : public CKXPluginGUI
{
public:
	iFreq_SplitterPlugin *plugin;

	// constructor:
	iFreq_SplitterPluginDlg(iFreq_SplitterPlugin *plg, kDialog *parent_, kFile *mf_):CKXPluginGUI(parent_, mf_) {plugin = plg;};
