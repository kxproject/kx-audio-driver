...
declare_effect_source(apscomp);

// Number of plugin parameters
#define APSCOMP_PARAMS_COUNT	6

// Plugin definition
class iAPSCompPlugin : public iKXPlugin
{
public:
// members of iKXPlugin
	virtual int request_microcode();
	virtual const char *get_plugin_description(int id) { plugin_description(apscomp); }
	
	virtual iKXPluginGUI *create_cp(kDialog *parent, kFile *);
	virtual int get_user_interface();

	virtual int get_param_count(void) {return APSCOMP_PARAMS_COUNT;}
...
// not members of iKXPlugin
	kxparam_t _params[APSCOMP_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iAPSCompPluginDlg : public CKXPluginGUI
{
public:
	iAPSCompPlugin *plugin;

    // constructor:
	iAPSCompPluginDlg(iAPSCompPlugin *plg, kDialog *parent_, kFile *mf_):CKXPluginGUI(parent_, mf_) {plugin = plg;};
