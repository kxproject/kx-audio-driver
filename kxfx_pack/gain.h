...
declare_effect_source(gain);

// Number of plugin parameters
#define GAIN_PARAMS_COUNT	1

// Plugin definition
class iGainPlugin : public iKXPlugin
{
public:
	// members of iKXPlugin
	virtual int request_microcode();
	virtual const char *get_plugin_description(int id) { plugin_description(gain); }
	
	virtual iKXPluginGUI *create_cp(kDialog *parent, kFile *);
	virtual int get_user_interface();

	virtual int get_param_count(void) {return GAIN_PARAMS_COUNT;}
	...
	kxparam_t _params[GAIN_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iGainPluginDlg : public CKXPluginGUI
{
public:
	iGainPlugin *plugin;

    // constructor:
	iGainPluginDlg(iGainPlugin *plg, kDialog *parent_, kFile *mf_):CKXPluginGUI(parent_, mf_) {plugin = plg;};
