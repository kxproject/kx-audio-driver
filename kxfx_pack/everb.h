...
declare_effect_source(everb);

// Number of plugin parameters
#define EVERB_PARAMS_COUNT	8

// Plugin definition
class iEverbPlugin : public iKXPlugin
{
public:
// members of iKXPlugin
	virtual int request_microcode();
	virtual const char *get_plugin_description(int id) { plugin_description(everb); }
	
	virtual iKXPluginGUI *create_cp(kDialog *parent, kFile *);
	virtual int get_user_interface();

	virtual int get_param_count(void) {return EVERB_PARAMS_COUNT;}
...
// not members of iKXPlugin
	kxparam_t _params[EVERB_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iEverbPluginDlg : public CKXPluginGUI
{
public:
	iEverbPlugin *plugin;

    // constructor:
	iEverbPluginDlg(iEverbPlugin *plg, kDialog *parent_, kFile *mf_):CKXPluginGUI(parent_, mf_) {plugin = plg;};
