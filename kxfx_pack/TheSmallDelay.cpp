...
// -------------------------
static kxparam_t default_params[THEDELAY_PARAMS_COUNT] = {1};
int iTheSmallDelayPlugin::set_defaults() 
{
...
static kx_fxparam_descr _param_descr[THEDELAY_PARAMS_COUNT] = {
    {"DelayTime", KX_FXPARAM_LEVEL, 0x1,    0x4B0}
    };

int iTheSmallDelayPlugin::describe_param(int ndx, kx_fxparam_descr *descr)
{
...
const kxparam_t *iTheSmallDelayPlugin::get_plugin_presets()
{
 return presets;
}
...
int iTheSmallDelayPlugin::get_param(int ndx, kxparam_t *value) // ndx corresponds to 'logical' parameter id
{
    *value = _params[ndx]; 
    return 0;
}
...
int iTheSmallDelayPlugin::set_param(int ndx, kxparam_t value)
{
    kxparam_t t;
    _params[ndx] = value;
    ...
    if (cp) ((iTheSmallDelayPluginDlg*) cp)->sync(ndx);

    return 0;
}

int iTheSmallDelayPlugin::set_all_params(kxparam_t* values)
{
    for (int i = 0; i < THEDELAY_PARAMS_COUNT; i++) set_param(i, values[i]);
    return 0;
}

int iTheSmallDelayPlugin::request_microcode() 
{
    publish_microcode(TheSmallDelay);

    return 0;
}

const char *iTheSmallDelayPlugin::get_plugin_description(int id)
{
    plugin_description(TheSmallDelay);
}


iKXPluginGUI *iTheSmallDelayPlugin::create_cp(kDialog *parent, kFile *mf)
{
    iTheSmallDelayPluginDlg *tmp;
    tmp = new iTheSmallDelayPluginDlg(this, parent,mf);
    tmp->create();
    return tmp;
}

int iTheSmallDelayPlugin::get_user_interface()
{
...
void iTheSmallDelayPluginDlg::init()
{
    CKXPluginGUI::init(); // this is necessary
    
    
    // dialog initialization
    create_dialog(plugin_name(TheSmallDelay), 500, 80);

    // slider
    create_hslider(dt_value, DT_ID, "Left", 1, 1200, 6, 10, 480, 30);
...
void iTheSmallDelayPluginDlg::sync(int ndx)
{
