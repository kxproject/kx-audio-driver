...
declare_effect_source(apsexp_plus);

// Number of plugin parameters
#define APSEXP_PLUS_PARAMS_COUNT	7

// Plugin definition
class iAPSExpPlusPlugin : public iKXPlugin
{
public:
// members of iKXPlugin
	virtual int request_microcode();
	virtual const char *get_plugin_description(int id) { plugin_description(apsexp_plus); }
	
	virtual iKXPluginGUI *create_cp(kDialog *parent, kFile *);
	virtual int get_user_interface();

	virtual int get_param_count(void) {return APSEXP_PLUS_PARAMS_COUNT;}
...
// not members of iKXPlugin
	kxparam_t _params[APSEXP_PLUS_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iAPSExpPlusPluginDlg : public CKXPluginGUI
{
public:
	iAPSExpPlusPlugin *plugin;

    // constructor:
	iAPSExpPlusPluginDlg(iAPSExpPlusPlugin *plg, kDialog *parent_, kFile *mf_):CKXPluginGUI(parent_, mf_) {plugin = plg;};
