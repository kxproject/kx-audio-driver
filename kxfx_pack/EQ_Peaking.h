...
declare_effect_source(EQ_Peaking);

// Number of plugin parameters
#define EQ_Peaking_PARAMS_COUNT	3

// Plugin definition
class iEQ_PeakingPlugin : public iKXPlugin
{
public:
	// members of iKXPlugin
	virtual int request_microcode();
	virtual const char *get_plugin_description(int id) { plugin_description(EQ_Peaking); };
	
	virtual iKXPluginGUI *create_cp(kDialog *parent, kFile *);
	virtual int get_user_interface();

	virtual int get_param_count(void) {return EQ_Peaking_PARAMS_COUNT;}
	...
	kxparam_t _params[EQ_Peaking_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iEQ_PeakingPluginDlg : public CKXPluginGUI
{
public:
	iEQ_PeakingPlugin *plugin;

	// constructor:
	iEQ_PeakingPluginDlg(iEQ_PeakingPlugin *plg, kDialog *parent_, kFile *mf_):CKXPluginGUI(parent_, mf_) {plugin = plg;};
