...
#define TIMBRE_PARAMS_COUNT	2

// Plugin definition
class iTimbrePlugin : public iKXPlugin
{
public:
// members of iKXPlugin
	...
	virtual int get_param_count(void) {return TIMBRE_PARAMS_COUNT;}
...
// not members of iKXPlugin
	kxparam_t _params[TIMBRE_PARAMS_COUNT]; // place to store parameters inside the plugin
};

// Plugin GUI definition
class iTimbrePluginDlg : public CKXPluginGUI
{
public:
	iTimbrePlugin *plugin;

    	// constructor:
	iTimbrePluginDlg(iTimbrePlugin *plg, kDialog *parent_, kFile *mf_) : CKXPluginGUI(parent_,mf_) {plugin = plg; };
